...
export async function POST(request: Request) {
  ...
  try {
    ...
    await env.DB.prepare("INSERT INTO post_recommendations(post_id,user_id,vote_type,created_at) VALUES(?,?,?,?)").bind(postId, user.id, vote, new Date().toISOString()).run();
    await env.DB.prepare(vote === "up" ? "UPDATE posts SET likes=likes+1 WHERE id=?" : "UPDATE posts SET dislikes=dislikes+1 WHERE id=?").bind(postId).run();
