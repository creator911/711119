...
export async function POST(request: Request) {
  ...
  try {
    ...
    await env.DB.prepare("INSERT INTO post_reports(post_id,user_id,reason,created_at) VALUES(?,?,?,?)").bind(postId, user.id, reason, new Date().toISOString()).run();
    await env.DB.prepare("UPDATE posts SET report_count=report_count+1 WHERE id=?").bind(postId).run();
