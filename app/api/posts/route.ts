...
import { maybePruneStalePreparedContent, publishPostWithReward } from "../../lib/content-rewards";
import {
...
export async function GET(request: Request) {
  ...
  const beforeId = Number.isInteger(requestedCursor) && requestedCursor > 0 ? requestedCursor : null;
  if (!validCategory(category)) return Response.json({ error: "게시판 종류를 확인해 주세요." }, { status: 400 });
  try {
    const { sql, bindings } = buildPostListQuery(category, sort, new Date(), { limit, beforeId });
    const statement = env.DB.prepare(sql);
    const posts = bindings.length ? await statement.bind(...bindings).all() : await statement.all();
    const visible = posts.results.map((post) => exposeCommunityTags(post as Record<string, unknown>));
    const nextCursor = sort === "latest" && visible.length === limit
      ? Number((visible.at(-1) as { id?: unknown })?.id ?? 0) || null
      : null;
    return Response.json({ posts: visible, nextCursor });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  const distributedLimit = await consumeDistributedRateLimit(env.CACHE, "post-create", String(user.id), 60, 60);
  ...
  try {
    ...
    await maybePruneStalePreparedContent(env.DB, postId).catch((cleanupError) => {
    ...
    const reward = await publishPostWithReward(env.DB, {
      postId,
      authorId: user.id,
      category,
      points: category === "reviews" ? pointSettings.reviewCreatePoints : pointSettings.postCreatePoints,
      createdAt,
      afterPublishStatements: bodyMediaFinalizeStatements(env.DB, mediaClaim, "post", postId, finalBody, createdAt),
    });
    saveCommitted = true;
    ...
    return Response.json({
      ...
      earnedPoints: reward.earnedPoints,
    }, { status: 201 });
  } catch (error) {
    if (!saveCommitted && createdPostId) {
      await env.DB.batch([
        env.DB.prepare(`
          DELETE FROM post_poll_votes WHERE poll_id IN (
            SELECT pp.id FROM post_polls pp JOIN posts p ON p.id=pp.post_id
            WHERE p.id=? AND p.status='draft'
          )
        `).bind(createdPostId),
        env.DB.prepare(`
          DELETE FROM post_poll_options WHERE poll_id IN (
            SELECT pp.id FROM post_polls pp JOIN posts p ON p.id=pp.post_id
            WHERE p.id=? AND p.status='draft'
          )
        `).bind(createdPostId),
        env.DB.prepare(`
          DELETE FROM post_polls WHERE post_id IN (
            SELECT id FROM posts WHERE id=? AND status='draft'
          )
        `).bind(createdPostId),
        env.DB.prepare("DELETE FROM posts WHERE id=? AND status='draft'").bind(createdPostId),
      ]).catch(() => undefined);
    }
