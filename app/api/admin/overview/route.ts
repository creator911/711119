...
export async function GET(request: Request) {
  ...
  const [members, posts, blockedIps, totalMembers, activeMembers, todayMembers, todayPosts, todayAttendance, supportUnread, partnerUnread, shopLowStockProducts] = await Promise.all([
    env.DB.prepare("SELECT id,username,nickname,signup_ip AS signupIp,first_login_ip AS firstLoginIp,points,level,is_director AS isDirector,is_partner AS isPartner,status,created_at AS createdAt FROM users ORDER BY id DESC LIMIT 200").all<AdminMemberRow>(),
    env.DB.prepare("SELECT p.id,p.category,p.title,p.views,p.likes,p.is_notice AS isNotice,p.status,p.created_at AS createdAt,CASE WHEN u.nickname IS NULL THEN '운영자' ELSE u.nickname END AS author FROM posts p LEFT JOIN users u ON u.id = p.author_id ORDER BY p.id DESC LIMIT 100").all(),
