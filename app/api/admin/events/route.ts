...
export async function POST(request: Request) {
  ...
  let createdPostId = 0;
  try {
    const payload = await request.json() as { title?: unknown; titleColor?: unknown; authorName?: unknown; body?: unknown };
    ...
    const inserted = await env.DB.prepare(`
      INSERT INTO posts(category,title,title_color,body,author_id,author_name,views,likes,status,created_at)
      VALUES('events',?,?,?,0,?,0,0,'published',?)
    `).bind(title, titleColor, body, authorName, createdAt).run();
    ...
    return Response.json({
      post: { id: postId, category: "events", title, titleColor, body: finalBody, author: authorName, authorLevel: 0, views: 0, likes: 0, status: "published", createdAt },
    }, { status: 201 });
  } catch (error) {
