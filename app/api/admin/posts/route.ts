...
export async function POST(request: Request) {
  ...
  let createdPostId = 0;
  try {
    const payload = await request.json() as { category?: unknown; title?: unknown; body?: unknown };
    ...
    const inserted = await env.DB.prepare(`
      INSERT INTO posts(category,title,body,author_id,views,likes,dislikes,report_count,is_notice,status,created_at)
      VALUES(?,?,?,0,0,0,0,0,?,'published',?)
    `).bind(category, title, body, isNotice, createdAt).run();
    ...
    return Response.json({
      post: { id: postId, category, title, body: finalBody, author: "운영자", authorLevel: 0, views: 0, likes: 0, dislikes: 0, reportCount: 0, commentCount: 0, isNotice: Boolean(isNotice), status: "published", createdAt },
    }, { status: 201 });
  } catch (error) {
