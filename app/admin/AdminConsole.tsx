...
type AdminTab = "posts" | "events" | "notices" | "shop" | "points" | "members" | "security" | "support" | "partner" | "directors" | "affiliates" | "domain";
...
const adminTitles: Record<AdminTab, string> = { posts: "최신글 리스트", events: "이벤트 관리", notices: "공지 관리", shop: "상점 수정", points: "포인트 지급 시스템", members: "회원정보 관리", security: "보안·IP 관리", support: "고객센터 상담", partner: "제휴문의 상담", directors: "실장", affiliates: "제휴회원", domain: "메인페이지 도메인" };
...
export default function AdminConsole() {
  ...
  return <div className="admin-shell">
    <aside className="admin-side"><div className="admin-brand"><span>CN</span><b>운영 콘솔</b></div><nav><button className={tab === "posts" ? "active" : ""} onClick={() => setTab("posts")}><span>01</span>최신글</button><button className={tab === "events" ? "active" : ""} onClick={() => setTab("events")}><span>02</span>이벤트 관리</button><button className={tab === "notices" ? "active" : ""} onClick={() => setTab("notices")}><span>03</span>공지 관리</button><button className={tab === "shop" ? "active" : ""} onClick={() => setTab("shop")}><span>04</span>상점 수정{overview.stats.shopLowStockProducts > 0 && <em title="자동상품 지급 이미지 확인 필요">{overview.stats.shopLowStockProducts > 99 ? "99+" : overview.stats.shopLowStockProducts}</em>}</button><button className={tab === "points" ? "active" : ""} onClick={() => setTab("points")}><span>05</span>포인트 지급</button><button className={tab === "members" ? "active" : ""} onClick={() => setTab("members")}><span>06</span>회원 관리</button><button className={tab === "security" ? "active" : ""} onClick={() => setTab("security")}><span>07</span>보안·IP</button><button className={tab === "support" ? "active" : ""} onClick={() => setTab("support")}><span>08</span>고객센터{overview.stats.supportUnread > 0 && <em>{overview.stats.supportUnread > 99 ? "99+" : overview.stats.supportUnread}</em>}</button><button className={tab === "partner" ? "active" : ""} onClick={() => setTab("partner")}><span>09</span>제휴문의{overview.stats.partnerUnread > 0 && <em>{overview.stats.partnerUnread > 99 ? "99+" : overview.stats.partnerUnread}</em>}</button><button className={tab === "directors" ? "active" : ""} onClick={() => setTab("directors")}><span>10</span>실장</button><button className={tab === "affiliates" ? "active" : ""} onClick={() => setTab("affiliates")}><span>11</span>제휴회원</button><button className={tab === "domain" ? "active" : ""} onClick={() => setTab("domain")}><span>12</span>메인페이지 도메인</button></nav><div className="admin-user"><div>10</div><p><b>{overview.operator.username}</b><small>{overview.operator.role === "owner" ? "OWNER · Lv.10" : "ADMIN · Lv.10"}</small></p></div></aside>
