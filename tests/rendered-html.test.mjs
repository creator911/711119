...
test("업체정보 행 전체가 열리고 게시글 목록은 밝은 줄무늬로 구분된다", async () => {
  ...
  assert.match(portal, /aria-label=\{`\$\{post\.industry\} \$\{post\.region\} \$\{post\.district\} \$\{post\.title\} 업체정보 보기`\}/);
...
test("모바일 메인에서 실시간 후기와 커뮤니티 카드를 모두 표시한다", async () => {
  ...
  assert.match(portal, /<BoardPreview title="실시간 후기"/);
  assert.match(portal, /<BoardPreview title="커뮤니티"/);
