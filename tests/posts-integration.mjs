...
for (const [label, category, communityTags] of [
  ["누락", "community", undefined],
  ["빈 배열", "community", []],
  ["기존 짤공유 누락", "gifs", undefined],
  ["허용되지 않은 값", "community", ["일상", "광고"]],
  ["중복", "community", ["꿀팁", "꿀팁"]],
]) {
  const invalidCommunityTags = await fetch(`${baseUrl}/api/posts`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Cookie: memberCookie },
    body: JSON.stringify({ category, communityTags, title: `머릿글 ${label} ${unique}`, body: "등록되면 안 됩니다." }),
  });
  assert.equal(invalidCommunityTags.status, 400, `${label} 머릿글 입력은 거부되어야 합니다.`);
}

const tagsOnReview = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "reviews", communityTags: ["일상"], title: `후기 머릿글 거부 ${unique}`, body: "등록되면 안 됩니다." }),
});
assert.equal(tagsOnReview.status, 400);
...
const pollPostResponse = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "community", communityTags: ["이슈", "꿀팁"], title: `게시글 투표 검증 ${unique}`, body: `<p>투표 위 문장</p><blockquote class="editor-poll-card" data-poll-config="${pollConfig}"><strong>VOTE</strong><h4>선호하는 이용 시간은 언제인가요?</h4><ol><li>오전</li><li>오후</li><li>저녁</li></ol></blockquote><p>투표 아래 문장</p>` }),
});
...
assert.deepEqual(pollPost.communityTags, ["꿀팁", "이슈"]);
const pollBefore = await (await fetch(`${baseUrl}/api/posts/${pollPost.id}`, { headers: { Cookie: voter.cookie } })).json();
assert.deepEqual(pollBefore.post.communityTags, ["꿀팁", "이슈"]);
...
const youtubePostResponse = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "community", communityTags: ["일상"], title: `유튜브 미리보기 검증 ${unique}`, body: `${youtubeEmbed}<iframe src="https://example.com/unsafe"></iframe>` }),
});
...
const invalidCommunityEdit = await fetch(`${baseUrl}/api/posts/${youtubePost.id}`, {
  method: "PATCH",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ title: youtubePost.title, body: youtubePost.body, communityTags: [] }),
});
assert.equal(invalidCommunityEdit.status, 400);

const communityTagEdit = await fetch(`${baseUrl}/api/posts/${youtubePost.id}`, {
  method: "PATCH",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ title: youtubePost.title, body: youtubePost.body, communityTags: ["유머", "후방"] }),
});
assert.equal(communityTagEdit.status, 200, JSON.stringify(await communityTagEdit.clone().json()));
assert.deepEqual((await communityTagEdit.json()).post.communityTags, ["후방", "유머"]);

const communityTagPreserved = await fetch(`${baseUrl}/api/posts/${youtubePost.id}`, {
  method: "PATCH",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ title: youtubePost.title, body: youtubePost.body }),
});
assert.equal(communityTagPreserved.status, 200, JSON.stringify(await communityTagPreserved.clone().json()));
assert.deepEqual((await communityTagPreserved.json()).post.communityTags, ["후방", "유머"]);
...
const deletionTarget = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "community", communityTags: ["일상"], title: `관리자 삭제 대상 ${unique}`, body: "관리자만 삭제할 수 있어야 합니다." }),
});
...
const ordinaryPin = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "community", communityTags: ["이슈"], title: `일반회원 고정 시도 ${unique}`, body: "레벨 10이 아니면 상단 고정을 사용할 수 없어야 합니다.", isPinned: true }),
});
...
const ownerEditData = await ownerEdit.json();
assert.equal(ownerEditData.post.title, ownerEditedTitle);
assert.deepEqual(ownerEditData.post.communityTags, ["일상"]);

const ownerDeleteTarget = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: memberCookie },
  body: JSON.stringify({ category: "community", communityTags: ["일상"], title: `오너 삭제 대상 ${unique}`, body: "오너 관리자 쿠키 삭제 검증용 게시글입니다." }),
});
...
const pinnedCommunity = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: voter.cookie },
  body: JSON.stringify({ category: "community", communityTags: ["꿀팁"], title: `커뮤니티 상단 고정 ${unique}`, body: "레벨 10 관리자가 등록한 커뮤니티 고정글입니다.", isPinned: true }),
});
...
const regularCommunity = await fetch(`${baseUrl}/api/posts`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Cookie: voter.cookie },
  body: JSON.stringify({ category: "community", communityTags: ["일상"], title: `최신 일반글 ${unique}`, body: "고정글보다 나중에 작성된 일반 커뮤니티 글입니다." }),
});
...
assert.deepEqual(communityOrder.posts.find((post) => post.id === pinnedCommunityData.post.id)?.communityTags, ["꿀팁"]);
