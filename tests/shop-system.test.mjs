...
test("상품 최소 레벨 미만 회원의 구매는 서버에서 차단된다", async () => {
  const database = await migratedDatabase();
  database.prepare(`
    INSERT INTO users(username,nickname,password_hash,password_salt,signup_ip,points,level,is_director,is_partner,role,status,created_at)
    VALUES(?,?,?,?,?,100000,2,0,0,'member','active',?)
  `).run("shoplevel", "레벨구매검증", "hash", "salt", "127.0.0.4", new Date().toISOString());
  const userId = Number(database.prepare("SELECT id FROM users WHERE username='shoplevel'").get().id);
  const product = database.prepare("SELECT id,name,price,stock FROM shop_products WHERE id=1").get();
  database.prepare("UPDATE shop_products SET min_level=3 WHERE id=?").run(product.id);

  assert.throws(() => database.prepare(`
    INSERT INTO shop_purchases(request_key,product_id,user_id,product_name,price,status,created_at)
    VALUES('shop-level-request-001',?,?,?,?,'pending_delivery',?)
  `).run(product.id, userId, product.name, product.price, new Date().toISOString()), /shop_level_insufficient/);
  assert.equal(database.prepare("SELECT COUNT(*) AS count FROM shop_purchases WHERE user_id=?").get(userId).count, 0);
  assert.equal(database.prepare("SELECT points FROM users WHERE id=?").get(userId).points, 100000);
  assert.equal(database.prepare("SELECT stock FROM shop_products WHERE id=?").get(product.id).stock, product.stock);

  database.prepare("UPDATE users SET level=3 WHERE id=?").run(userId);
  database.prepare(`
    INSERT INTO shop_purchases(request_key,product_id,user_id,product_name,price,status,created_at)
    VALUES('shop-level-request-002',?,?,?,?,'pending_delivery',?)
  `).run(product.id, userId, product.name, product.price, new Date().toISOString());
  assert.equal(database.prepare("SELECT COUNT(*) AS count FROM shop_purchases WHERE user_id=?").get(userId).count, 1);
  database.close();
});
